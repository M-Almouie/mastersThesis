...
def show_progress(session, epoch, feed_dict_train, feed_dict_validate, accuracy, merged, 
                    val_accuracy, val_loss, train_writer, test_writer, i):
    msg = "Training Epoch {0}, Training Accuracy: {1:>6.1%}, Validation Accuracy: {2:>6.1%},  Validation Loss: {3:.3f}"
    summary_train, train_acc = session.run([merged, accuracy], feed_dict=feed_dict_train)
...
def train(session, data, x, y_true, cost, optimiser, accuracy, merged, val_accuracy,
            train_writer, test_writer, num_iteration):
    total_iterations = 0
    ...
    for i in range(total_iterations,
        ...
        if i % int(data.train.num_examples/batch_size) == 0: 
            ...
            show_progress(session, epoch, feed_dict_tr, feed_dict_val, accuracy, merged, 
                            val_accuracy, val_loss, train_writer, test_writer, i) 

            #runActivationOnLayerN(layer_conv1)
    total_iterations += num_iteration

def getActivations(session, layer, x, stimuli):
...
def runActivationOnLayerN(session, layer, x):
    ...
    getActivations(session, layer, x, imageToUse)
...
def main():
    ...
    train_writer = tf.summary.FileWriter('summary/trainModel7', session.graph)
    test_writer = tf.summary.FileWriter('summary/testModel7')
    ...
    saver.save(session, './retina-model7')
